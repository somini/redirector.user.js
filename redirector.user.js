...
var redirs = new Object();
	...
	*/
	...
	/* Play4Real Comments
	 * Redirects the P4R comments feed to the correct page, to workaround their cache bug
	 */
	redirs.p4r_comments = new Object();
	redirs.p4r_comments.active = true;
	redirs.p4r_comments.removePath_regex = /comment-page-[0-9]+\/$/;
...
if (window.top == window.self ) { //only on the top window
	...
	/* P4R Comments */
	if ( redirs.p4r_comments.active && /p4rgaming\.com/.test(oldURL.host()) ) {
		newURL.path( oldURL.path().replace(redirs.p4r_comments.removePath_regex,"") );
	}
